    ...
    try:
        ...
        if creation_result["success"]:
            ...
            if creation_result.get("needs_confirmation"):
                ...
                # Обновляем результат
                result.success = True
                result.summary = f"✅ Создано {len(creation_result['created_stories'])} User Stories из статьи Confluence"
                result.message = f"✅ Создано {len(creation_result['created_stories'])} User Stories из статьи Confluence"
                result.processing_end = datetime.now()
                result.duration_seconds = (result.processing_end - result.processing_start).total_seconds()
...
async def handle_user_confirmation(request: UserRequest, result: DetailedProcessingResult) -> DetailedProcessingResult:
    ...
    try:
        ...
        if creation_result["success"]:
            ...
            # Обновляем результат
            result.success = True
            result.summary = detailed_message
